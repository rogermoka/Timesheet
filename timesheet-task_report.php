        ...
        <div class="row">
            <div class="col-md-12 table-title">
                <section class="col-md-12 heading">
                    ...
                    <h2 class="col-md-4">TASK TIMESHEET REPORT</h2>
                    <div class="col-md-4 nav">
                        <? printPrevNext($time, $next_week, $prev_week, $next_month, $prev_month, $time_middle_month,"proj_id=$proj_id", $mode);?>
                        <div class="downLoad">
                            <a href="csv/timesheet-task_report.csv"><i class="fa fa-download" aria-hidden="true"></i> CSV</a>
                        </div>
                    </div>
                </section>
                    <?php
                    ...
                    if ( !empty($tasksArr) ) {
                    	foreach ($tasksArr as $key => $value) {
                    		...
                    		$i = 0;
                    		while ($i < $count) { 
                    			$taskFinal[$key][] = array($value['users'][$i] => $value['time'][$i]);
                    			$i++;
                    		}
                        } ?>
                        <table class="table table-bordered table-fill">
                            ...
                            foreach ($taskFinal as $key => $value) {
                                $newVar = array();
                                echo "<tr>";
                                    ...
                                    foreach ($users as $uKey => $uValue) {
                                       		...
                                       		while($cI < $countNum){
                                       			$uTotal += @$value[$cI][$uValue['username']];
                                                $cI++;
                                            }

                                            $colTotal += $uTotal;
                                            $rowTotal[$uValue['username']][] =  $uTotal;

                                            echo "<td align='center'>";
                                            if ($uTotal) {
                                                echo format_seconds($uTotal);
                                                $newVar[] = format_seconds($uTotal);
                                            }else{
                                                $newVar[] = "--";
                                            }
                                            echo "</td>";
                            ...
                            <tr class="grand_total">
                                ...
                                <?php 
                                    ...
                                    $filename = "csv/timesheet-task_report.csv";
                                    $fp = fopen($filename, 'w');
