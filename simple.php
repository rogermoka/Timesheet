...
<!-- Custom code : Structure for send mail -->
<?php $countSheets = 0; ?>
<div style="display: none" id="contentFetch" class="mailContentContainer">
<h3 style="color:#433B94; padding-bottom: 5px; font-family: arial; border-bottom: 1px solid #433B94;">Week Start: <? echo date('D F j, Y',mktime(0,0,0,$startMonth, $startDay, $startYear)); ?></h3>
<table style="font-family: arial; font-size: 11px;border: 1px solid #433B94;" width="100%" align="center" border="0" cellpadding="0" cellspacing="0">
		<tr style="background-color: #433B94; color: #fff;" >
						<td width="300" style="padding: 5px 0; font-size: 12px; color: #fff;" align="center">Project / Task</td>
						<td width="2">&nbsp;</td>
						<?php
						//print the days of the week
						$currentDayDate = $startDate;
						...
						?>
						<td style="color: #fff;" align="center" width="50">Total</td>
					</tr>
					<tr>
<?

$nCount = count($structuredArray);
unset($newMatchedPair);
for ($i=0; $i<$nCount; $i++) {
		$countSheets++;
		$newMatchedPair = &$structuredArray[$i];
	?><tr id="row<?php echo $i; ?>"><?
		?><td style="border-bottom: 1px solid #433B94;" valign="top"><?
			?><table width="100%" border="0" cellspacing="0" cellpadding="0"><?
				?><tr><?
						?><td width="50%" align="left"><?
							echo "<p style='font-size:12px; color:#433B94; padding:10px;'>".$newMatchedPair->projectTitle."</p>";
						?></td><?
						?><td width="50%" align="left"><?
							echo "<p style='font-size:12px; color:#433B94; padding:10px;'>".$newMatchedPair->taskName."</p>";
						?></td><?
				?></tr><?				
				?><tr><?
						?><td height="1"></td><?
		...
		//print the spacer column
		print "<td style=\"border-bottom: 1px solid #433B94;\" width=\"2\">&nbsp;</td>";
		
		//iterate through the days array
		$currentDay = 0;
		$weeklyTotal = 0;
		foreach ($newMatchedPair->value2 as $currentDayArray) {
			//open the column
			print "<td style=\"border-bottom: 1px solid #433B94;\" align=\"left\">";
			...
			//make sure the cell has at least a space in it so that its rendered by the browser
			if ($emptyCell) {
				print "<span nowrap>0h </span>";
				print "<span nowrap>0m</span>";			
			}
			else {
				//split todays total into hours and minutes
				$todaysHours = floor($todaysTotal / 60 / 60);
				$todaysMinutes = ($todaysTotal - ($todaysHours * 60 * 60)) / 60;
				print "<span nowrap>".$todaysHours ."h </span>";
				print "<span nowrap>" . $todaysMinutes . "m</span>";			
			}
		...
		//print the total column
		print "<td style=\"border-bottom: 1px solid #433B94;color:#ff0000\" align=\"right\" ><b style='margin-right:10px;'>";
		print $weeklyTotalStr."<b></td>";
		...
		//store the previous task and project ids
		$previousTaskId = $currentTaskId;
		$previousProjectId = $newMatchedPair->projectId;
	}

		//create a new totals row
	print "<tr id=\"totalsRow\">\n";
	print "<td style='padding:5px 0;' align=\"right\"><span style='margin-right:10px;'>Total Hours:</span></td>\n";
	print "<td style='padding:5px 0; margin-right:10px;' width=\"2\">&nbsp;</td>\n";
						
	//iterate through day totals for all tasks
	$NewgrandTotal = 0;
	$Newcol = 0;

	foreach ($allTasksDayTotals as $NewcurrentAllTasksDayTotal) {
		$NewcurrentAllTasksDayTotal = $NewcurrentAllTasksDayTotal/2;
		$Newcol++;
		$NewgrandTotal += $NewcurrentAllTasksDayTotal;
		$NewformattedTotal = formatSeconds($NewcurrentAllTasksDayTotal);
		print "<td style='padding:5px 0;'>\n";
		print "<b style='color:#ff0000' >$NewformattedTotal</b></td>";
	}
	
	//print grand total
	$NewformattedGrandTotal = formatSeconds($NewgrandTotal);
	print "<td style='padding:5px 0;' align=\"right\">\n";
	print "<b style='margin-right:5px;color:#ff0000'>$NewformattedGrandTotal</b></td>";
	print "</tr>";

?>
</table></div>
<div id="sheetsExists" data-sheets="<?= $countSheets ?>"></div>
<?php /*  End mail structure here */ 
